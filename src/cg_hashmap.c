 ...
 */
static map_ssize_t 
cgi_name_lookup(cgns_hashmap_object *mp, char *key,
...
*/
static int
cgi_insert_key(cgns_hashmap_object *mp, char *key, map_ssize_t hash, map_ssize_t value)
{
...
static int
cgi_insert_to_emptymap(cgns_hashmap_object *mp, char *key, map_ssize_t hash, map_ssize_t value)
{
...
map_ssize_t
cgi_map_get_item(cgns_hashmap_object *op, char *key)
{
    map_ssize_t ix;
    map_ssize_t hash;
    map_ssize_t value;
...
int
cgi_map_set_item(cgns_hashmap_object *op, char *key, map_ssize_t value)
{
    map_ssize_t hash;
    if (op == NULL) {
        return -1;
...
int
cgi_map_contains(cgns_hashmap_object *op, char *key)
{
    map_ssize_t hash;
    map_ssize_t ix;
    map_ssize_t value;
...
int
_cg_del_shift_item_known_hash(cgns_hashmap_object* op, char* key, map_ssize_t hash)
{
    map_ssize_t ix;
    cgns_hashmap_entry* ep;
...
int
cgi_map_del_shift_item(cgns_hashmap_object* op, char* key)
{
    map_ssize_t hash;
    assert(key);
    hash = cgi_hash_cstr(key);
